...
export function SuratJalanModule() {
  ...
  const fetchAllData = async () => {
    setLoading(true);
    try {
      // 1. Fetch Documents & Items separately to ensure reliable joins without 1000 limit
      const [docsData, itemsData] = await Promise.all([
        fetchAllRowsFromSupabase('documents', { orderBy: 'created_at', ascending: false }),
  ...
  return (
    <div className="space-y-2.5 sm:space-y-3 text-slate-800">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2 p-2 sm:p-2.5 bg-white border border-slate-200 rounded-xl shadow-2xs">
        ...
        <button
          ...
          disabled={loading}
          className="p-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold transition-all cursor-pointer flex items-center gap-1"
        ...
        <div className="space-y-2.5">
          ...
          <div className="bg-white p-2.5 sm:p-3 rounded-xl border border-slate-200 shadow-2xs space-y-2.5">
            <div className="flex items-center justify-between flex-wrap gap-2">
              <div className="flex items-center gap-2 flex-wrap flex-1 min-w-[240px]">
                <div className="relative flex-1 min-w-[160px]">
                  ...
                  <input
                    ...
                    className="w-full pl-8 pr-3 py-1.5 bg-slate-50 border border-slate-300 rounded-lg text-xs font-semibold outline-none focus:ring-2 focus:ring-blue-500"
                  />
                </div>
                ...
                <select
                  ...
                  className="px-2.5 py-1.5 bg-slate-50 border border-slate-300 rounded-lg text-xs font-bold text-slate-700 outline-none focus:ring-2 focus:ring-blue-500"
                >
              ...
              <button
                onClick={handleOpenNewForm}
                className="px-3 py-1.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold shadow-xs transition-all cursor-pointer flex items-center gap-1.5"
              >
                <Plus size={15} />
                <span>Buat SJ Baru</span>
              </button>
            ...
            <div className="overflow-x-auto border border-slate-200 rounded-lg">
              <table className="w-full text-left text-xs border-collapse">
                <thead className="bg-slate-50 text-slate-700 font-extrabold uppercase border-b border-slate-200">
                  <tr>
                    <th className="p-2">Nomor SJ</th>
                    <th className="p-2">Tanggal</th>
                    <th className="p-2">Jenis</th>
                    <th className="p-2">Tujuan</th>
                    <th className="p-2">UP / Kontak</th>
                    ...
                    filteredDashDocs.map((doc) => {
                      ...
                      return (
                        <tr key={doc.id} className="hover:bg-blue-50/40 transition-colors">
                          <td className="p-2 font-mono font-bold text-amber-600 whitespace-nowrap">
                            {doc.nomorSJ}
                          </td>
                          <td className="p-2 whitespace-nowrap">{doc.tanggal}</td>
                          <td className="p-2 whitespace-nowrap">
                            <span className="bg-amber-100 text-amber-800 font-mono text-[10px] px-1.5 py-0.5 rounded border border-amber-200 font-bold">
                              {j ? j.kode : 'CKB'}
                            </span>
                          </td>
                          <td className="p-2 font-bold text-slate-900">{t ? t.nama : 'N/A'}</td>
                          <td className="p-2 text-slate-600">
                            {doc.up || '-'} {doc.noTelpon ? `(${doc.noTelpon})` : ''}
                          </td>
                          <td className="p-2 text-right font-mono font-bold">{doc.items.length} item</td>
                          <td className="p-2 text-right whitespace-nowrap">
                            <div className="flex items-center justify-end gap-1">
                              ...
                              <button
                                ...
                                className="p-1 rounded bg-slate-100 hover:bg-amber-100 text-slate-700 hover:text-amber-700 cursor-pointer transition-colors"
                                title="Edit"
                              >
                                <Edit size={13} />
                              </button>
      ...
      {activeTab === 'form' && (
        <div className="space-y-2.5">
          ...
          <div className="flex items-center justify-between bg-white p-2.5 sm:p-3 rounded-xl border border-slate-200 shadow-2xs">
            <div>
              <h2 className="text-sm sm:text-base font-black text-slate-900 uppercase m-0">
                {editDocId ? 'Edit Surat Jalan' : 'Buat Surat Jalan Baru'}
              </h2>
              <p className="text-[11px] text-slate-500 font-semibold m-0 mt-0.5">Isi rincian pengiriman dan daftar barang</p>
            </div>
            <button
              onClick={() => setActiveTab('dashboard')}
          ...
          <div className="bg-white p-2.5 sm:p-3 rounded-xl border border-slate-200 shadow-2xs space-y-2.5">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
              <div>
                ...
                <select
                  ...
                  className="w-full bg-white border border-slate-300 rounded-lg px-2.5 py-1.5 text-xs font-bold text-slate-900 focus:ring-2 focus:ring-blue-500 outline-none"
                >
                  {jenisList.map(j => (
                    <option key={j.id} value={j.id}>{j.nama} ({j.kode})</option>
                  ))}
            ...
            <div>
              ...
              <div className="p-2 bg-blue-50 border border-blue-200 rounded-lg font-mono text-xs font-black text-blue-900">
                {formState.nomorSJ}
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
              <div>
                <div className="flex items-center justify-between mb-1">
                  <label className="block text-[11px] font-extrabold text-slate-800 uppercase tracking-wider m-0">
                    Tujuan Pengiriman
                  </label>
                  <button
                    type="button"
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-[11px] font-extrabold text-slate-800 uppercase tracking-wider mb-1">UP (Penerima)</label>
                  <input
                    type="text"
                    value={formState.up}
                    onChange={(e) => setFormState({ ...formState, up: e.target.value })}
                    placeholder="Nama UP"
                    className="w-full bg-white border border-slate-300 rounded-lg px-2.5 py-1.5 text-xs font-bold outline-none focus:ring-2 focus:ring-blue-500"
                  />
                </div>
                <div>
                  <label className="block text-[11px] font-extrabold text-slate-800 uppercase tracking-wider mb-1">No. Telepon</label>
                  <input
                    type="text"
            ...
            {quickAddTujuanOpen && (
              <div className="p-3 bg-orange-50/80 border border-orange-200 rounded-lg space-y-2 animate-in fade-in duration-200">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                  <input
                    ...
                    className="px-2.5 py-1.5 bg-white border border-slate-300 rounded-lg text-xs font-semibold outline-none"
                  />
                  <input
                    type="text"
                ...
                <div className="flex justify-end gap-2">
                  ...
                  <button
                    ...
                    className="px-3.5 py-1 rounded-lg bg-blue-600 text-white text-xs font-bold shadow-xs"
                  >
                    Simpan Tujuan
                  </button>
                </div>
          ...
          <div className="bg-white p-2.5 sm:p-3 rounded-xl border border-slate-200 shadow-2xs space-y-2.5">
            <div className="flex items-center justify-between flex-wrap gap-2 pb-2 border-b border-slate-100">
              ...
              <div className="flex border border-slate-300 rounded-lg overflow-hidden p-0.5 bg-slate-100">
                <button
                  type="button"
                  onClick={() => setFormMode('manual')}
                  className={`px-2.5 py-1 rounded text-xs font-bold cursor-pointer ${formMode === 'manual' ? 'bg-blue-600 text-white shadow-xs' : 'text-slate-700'}`}
                >
                  Manual
                </button>
                <button
                  ...
                  className={`px-2.5 py-1 rounded text-xs font-bold cursor-pointer ${formMode === 'tempel' ? 'bg-blue-600 text-white shadow-xs' : 'text-slate-700'}`}
                >
                  Tempel (Paste)
                </button>
                <button
                  ...
                  className={`px-2.5 py-1 rounded text-xs font-bold cursor-pointer ${formMode === 'impor' ? 'bg-blue-600 text-white shadow-xs' : 'text-slate-700'}`}
                >
                  Impor File
                </button>
              </div>
              ...
              <div className="p-3 bg-slate-50 border border-slate-200 rounded-lg space-y-2">
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className={`px-2.5 py-1 rounded-md text-xs font-bold ${pasteSubMode === 'all' ? 'bg-blue-600 text-white' : 'bg-slate-200 text-slate-700'}`}
                  >
                  ...
                  <button
                    ...
                    className={`px-2.5 py-1 rounded-md text-xs font-bold ${pasteSubMode === 'column' ? 'bg-blue-600 text-white' : 'bg-slate-200 text-slate-700'}`}
                  >
                  ...
                  <div>
                    ...
                    <div className="flex justify-end mt-1.5">
                      <button
                        ...
                        className="px-3 py-1 rounded-lg bg-blue-600 text-white text-xs font-bold shadow-xs"
                      >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          className="w-full p-1.5 bg-white border border-slate-300 rounded-lg font-mono text-xs outline-none"
                        />
                      </div>
                    </div>
            ...
            {formMode === 'impor' && (
              <div className="p-3 bg-slate-50 border border-slate-200 rounded-lg space-y-1.5">
                ...
                <input
                  ...
                  className="block w-full text-xs text-slate-500 file:mr-2.5 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-bold file:bg-blue-600 file:text-white hover:file:bg-blue-700 cursor-pointer"
                />
              </div>
            )}
              ...
              <table className="w-full text-left text-xs border-collapse">
                <thead className="bg-slate-100 text-slate-700 font-extrabold uppercase border-b border-slate-200">
                  <tr>
                    ...
                    <th className="p-2">Nama Barang / Deskripsi</th>
                    <th className="p-2 w-20">CTN</th>
                    <th className="p-2 w-20">PCS</th>
                    <th className="p-2 w-24">Berat (KG)</th>
                    <th className="p-2 w-24">Kubikasi (M3)</th>
                    ...
                    <tr key={idx} className="hover:bg-slate-50">
                      ...
                      <td className="p-1">
                        <input
                          ...
                          className="w-full px-2 py-1 border border-slate-200 rounded-md text-xs font-bold outline-none focus:border-blue-500"
                        />
                      </td>
                      <td className="p-1">
                        <input
                          ...
                          className="w-full px-2 py-1 border border-slate-200 rounded-md text-xs font-bold text-right outline-none focus:border-blue-500"
                        />
                      </td>
                      <td className="p-1">
                        <input
                      ...
                      <td className="p-1">
                        <input
                          ...
                          placeholder="Ket..."
                          className="w-full px-2 py-1 border border-slate-200 rounded-md text-xs outline-none focus:border-blue-500"
                        />
                      </td>
            ...
            <div className="flex items-center justify-between">
              <button
                ...
                className="px-2.5 py-1 rounded-lg bg-blue-50 hover:bg-blue-100 text-blue-700 text-xs font-bold cursor-pointer flex items-center gap-1"
              >
                <Plus size={14} />
          ...
          <div className="flex justify-end gap-2 pt-1">
            ...
            <button
              ...
              className="px-4 py-1.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-xs font-extrabold shadow-xs transition-all cursor-pointer flex items-center gap-1.5"
            >
              <Save size={15} />
      ...
      {activeTab === 'print' && printDoc && (
        <div className="space-y-2.5">
          {/* Print Toolbar */}
          <div className="flex items-center justify-between bg-white p-2.5 sm:p-3 rounded-xl border border-slate-200 shadow-2xs flex-wrap gap-2 print:hidden">
            <button
              onClick={() => setActiveTab('dashboard')}
            ...
            <div className="flex items-center gap-2">
              <div className="flex border border-slate-300 rounded-lg overflow-hidden p-0.5 bg-slate-100">
                <button
                  ...
                  className={`px-2.5 py-1 rounded text-xs font-bold ${paperSize === 'A4' ? 'bg-blue-600 text-white' : 'text-slate-700'}`}
                >
                  A4
                </button>
                <button
                  onClick={() => setPaperSize('Letter')}
                  className={`px-2.5 py-1 rounded text-xs font-bold ${paperSize === 'Letter' ? 'bg-blue-600 text-white' : 'text-slate-700'}`}
                >
                  Letter
                </button>
              </div>
              ...
              <button
                ...
                className="px-3.5 py-1.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold shadow-xs transition-all flex items-center gap-1 cursor-pointer"
              >
                <Printer size={15} />
      ...
      {activeTab === 'rekap' && (
        <div className="bg-white p-2.5 sm:p-3 rounded-xl border border-slate-200 shadow-2xs space-y-2.5">
          <div className="flex items-center justify-between flex-wrap gap-2 pb-2 border-b border-slate-100">
            <div>
              <h2 className="text-sm sm:text-base font-black text-slate-900 uppercase m-0 flex items-center gap-1.5">
                <FileSpreadsheet className="text-blue-600" size={18} />
                <span>Rekapitulasi Barang Pengiriman</span>
              </h2>
              <p className="text-[11px] text-slate-500 font-semibold m-0 mt-0.5">Data seluruh rincian barang dari semua Surat Jalan</p>
            </div>

            <div className="flex items-center gap-1.5 flex-wrap">
              <span className="bg-blue-100 text-blue-800 text-[11px] font-bold px-2 py-1 rounded-lg border border-blue-200">
              ...
              <button
                ...
                className="px-2.5 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold shadow-xs transition-all cursor-pointer flex items-center gap-1"
              ...
              >
                <FolderCheck size={14} />
                <span>Riwayat ({savedRekapList.length})</span>
              </button>

              <button
                onClick={exportRekapExcel}
                className="px-3 py-1.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold shadow-xs transition-all cursor-pointer flex items-center gap-1"
              >
                <Download size={14} />
              ...
              {savedRekapList.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2 max-h-[300px] overflow-y-auto pr-1">
                  {savedRekapList.map((rk) => (
                    <div key={rk.id} className="p-2.5 bg-white border border-purple-200 rounded-lg shadow-2xs space-y-1.5 hover:border-purple-300 transition-all">
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <span className="px-1.5 py-0.5 bg-purple-100 text-purple-800 text-[10px] font-extrabold rounded whitespace-nowrap">
                          {rk.total_item || rk.data_detail?.length || 0} Barang
                        </span>
                      </div>

                      <div className="grid grid-cols-4 gap-1 bg-slate-50 p-1.5 rounded text-center text-[10px] font-bold text-slate-700">
                        <div>
                      ...
                      <div className="flex items-center justify-end gap-1 pt-0.5">
                        <button
                          ...
                          className="px-2 py-0.5 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded text-[11px] font-bold transition-all cursor-pointer flex items-center gap-0.5"
                        >
                          <Eye size={11} />
                          <span>Detail</span>
                        </button>
                        <button
                          ...
                          className="px-2 py-0.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 rounded text-[11px] font-bold transition-all cursor-pointer flex items-center gap-0.5"
                        >
                          <Download size={11} />
                          <span>Excel</span>
                        </button>
          ...
          <div className="overflow-x-auto border border-slate-200 rounded-lg max-h-[500px] overflow-y-auto">
            <table className="w-full text-left text-xs border-collapse">
              <thead className="bg-slate-100 text-slate-700 font-extrabold uppercase sticky top-0 z-10 border-b border-slate-200">
                <tr>
                  <th className="p-2 w-14 text-center">No Rekap</th>
                  <th className="p-2">Nomor SJ</th>
                  <th className="p-2">Tanggal</th>
                  <th className="p-2">Tujuan</th>
                  ...
                  <th className="p-2 text-right">Kubikasi (M3)</th>
                  <th className="p-2">Keterangan</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 font-medium text-slate-800">
                {filteredRekapItems.length === 0 ? (
                  ...
                  filteredRekapItems.map((it, idx) => (
                    <tr key={idx} className="hover:bg-blue-50/40 transition-colors">
                      <td className="p-2 text-center font-mono text-slate-400">{it.rekapNo}</td>
                      <td className="p-2 font-mono font-bold text-amber-600">{it.nomorSJ}</td>
                      <td className="p-2 whitespace-nowrap">{it.tanggal}</td>
                      <td className="p-2 font-semibold text-slate-900">{it.tujuan}</td>
                      <td className="p-2 font-semibold text-slate-900">{it.namaBarang}</td>
                      <td className="p-2 text-right font-mono font-bold">{formatNum(it.ctn)}</td>
                      <td className="p-2 text-right font-mono font-bold">{formatNum(it.pcs)}</td>
                      <td className="p-2 text-right font-mono">{it.berat.toFixed(2)}</td>
                      <td className="p-2 text-right font-mono">{it.kubikasi.toFixed(3)}</td>
                      <td className="p-2 text-slate-500">{it.keterangan || '-'}</td>
                    </tr>
                  ))
                )}
              </tbody>
        ...
        <div className="space-y-2.5">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5">
            ...
            <div className="bg-white p-2.5 sm:p-3 rounded-xl border border-slate-200 shadow-2xs space-y-2">
              <h3 className="text-xs sm:text-sm font-black uppercase text-slate-900 m-0 pb-1.5 border-b border-slate-100">
                Data Pengirim (Kop SJ)
              </h3>
              <div>
                <label className="block text-[11px] font-extrabold text-slate-700 uppercase mb-0.5">Nama Pengirim</label>
                <input
                  type="text"
                  value={pengirim.nama}
                  onChange={(e) => setPengirim({ ...pengirim, nama: e.target.value })}
                  className="w-full px-2.5 py-1.5 bg-slate-50 border border-slate-300 rounded-lg text-xs font-bold outline-none"
                />
              </div>

              <div>
                <label className="block text-[11px] font-extrabold text-slate-700 uppercase mb-0.5">Alamat Lengkap</label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-[11px] font-extrabold text-slate-700 uppercase mb-0.5">Kota / Kab</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-[11px] font-extrabold text-slate-700 uppercase mb-0.5">Telepon</label>
                  <input
                    type="text"
                    value={pengirim.telp}
              ...
              <button
                ...
                className="px-3 py-1.5 rounded-lg bg-blue-600 text-white text-xs font-bold shadow-xs cursor-pointer flex items-center gap-1 mt-1"
              >
                <Save size={14} />
            ...
            <div className="bg-white p-2.5 sm:p-3 rounded-xl border border-slate-200 shadow-2xs space-y-2">
              <div className="flex items-center justify-between pb-1.5 border-b border-slate-100">
                <h3 className="text-xs sm:text-sm font-black uppercase text-slate-900 m-0">
                  Jenis Surat Jalan ({jenisList.length})
                </h3>
                <button
                  type="button"
                  onClick={handleOpenAddJenis}
                  className="px-2.5 py-1 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold transition-all cursor-pointer flex items-center gap-1 shadow-xs"
                >
                  <Plus size={13} />
              ...
              <div className="space-y-1.5 max-h-[300px] overflow-y-auto pr-1">
                {jenisList.length === 0 ? (
                  ...
                  jenisList.map(j => (
                    <div key={j.id} className="flex items-center justify-between p-2 bg-slate-50 border border-slate-200 rounded-lg hover:border-slate-300 transition-colors">
                      <div>
                        <span className="text-xs font-bold text-slate-900 block">{j.nama}</span>
                        <span className="bg-amber-100 text-amber-800 font-mono text-[10px] px-1.5 py-0.5 rounded border border-amber-200 font-bold inline-block mt-0.5">
                          {j.kode}
                        </span>
                      </div>
                      <div className="flex items-center gap-1">
                        <button
                          ...
                          className="p-1 rounded bg-white hover:bg-amber-100 text-slate-600 hover:text-amber-700 cursor-pointer border border-slate-200"
                          title="Edit Jenis"
                        >
                          <Edit size={12} />
                        </button>
          ...
          <div className="bg-white p-2.5 sm:p-3 rounded-xl border border-slate-200 shadow-2xs space-y-2">
            <div className="flex items-center justify-between flex-wrap gap-2 pb-1.5 border-b border-slate-100">
              ...
              <button
                ...
                className="px-2.5 py-1.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold transition-all cursor-pointer flex items-center gap-1 shadow-xs"
              >
                <Plus size={14} />
            ...
            <div className="overflow-x-auto border border-slate-200 rounded-lg">
              <table className="w-full text-left text-xs border-collapse">
                ...
                <tbody className="divide-y divide-slate-100 font-semibold text-slate-800">
                  {tujuanList.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="p-5 text-center text-slate-400 font-bold">
                        Belum ada tujuan pengiriman. Klik "Tambah Tujuan" untuk menambah.
                      </td>
                    </tr>
                  ) : (
                    tujuanList.map(t => (
                      <tr key={t.id} className="hover:bg-slate-50">
                        ...
                        <td className="p-2 text-slate-600">{t.alamat || '-'}</td>
                        <td className="p-2 text-slate-700">{t.kota || '-'}</td>
                        <td className="p-2 text-slate-700">{t.up || '-'}</td>
                        <td className="p-2 text-slate-700">{t.noTelpon || '-'}</td>
                        <td className="p-2 text-right whitespace-nowrap">
                          <div className="flex items-center justify-end gap-1">
                            <button
                              onClick={() => handleOpenEditTujuanMaster(t)}
                              className="p-1 rounded bg-slate-100 hover:bg-amber-100 text-slate-700 hover:text-amber-700 cursor-pointer"
                              title="Edit Tujuan"
                            >
                              <Edit size={12} />
                            </button>
      ...
      {showJenisModal && typeof document !== "undefined" && createPortal(
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl p-4 sm:p-5 max-w-md w-full shadow-xl space-y-3 animate-in fade-in zoom-in-95">
            ...
            <div className="space-y-2.5">
              <div>
                ...
                <input
                  ...
                  className="w-full px-2.5 py-1.5 bg-slate-50 border border-slate-300 rounded-lg text-xs font-bold outline-none focus:ring-2 focus:ring-blue-500 uppercase"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-2.5 py-1.5 bg-slate-50 border border-slate-300 rounded-lg text-xs font-bold outline-none focus:ring-2 focus:ring-blue-500"
                />
              </div>
            </div>
            ...
            <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-100">
              ...
              <button
                ...
                className="px-4 py-1.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold shadow-xs transition-all cursor-pointer flex items-center gap-1"
              >
                <Save size={14} />
                <span>Simpan</span>
              </button>
      ...
      {showTujuanModal && typeof document !== "undefined" && createPortal(
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl p-4 sm:p-5 max-w-lg w-full shadow-xl space-y-3 animate-in fade-in zoom-in-95">
            <div className="flex items-center justify-between border-b pb-2.5 border-slate-100">
              <h3 className="text-sm font-black text-slate-900 uppercase m-0">
      ...
      {showSaveModal && typeof document !== "undefined" && createPortal(
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-md w-full p-4 sm:p-5 shadow-2xl border border-slate-100 space-y-3 animate-in fade-in zoom-in-95 duration-150">
            ...
            <div className="space-y-2.5 text-xs">
              <div>
                ...
                <input
                  ...
                  className="w-full px-2.5 py-1.5 bg-slate-50 border border-slate-300 rounded-lg text-xs font-bold text-slate-900 outline-none focus:ring-2 focus:ring-emerald-500"
                />
              </div>

              <div className="p-2.5 bg-emerald-50/70 border border-emerald-200 rounded-lg space-y-1 text-[11px] text-emerald-900 font-semibold">
            ...
            <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-100">
              ...
              <button
                ...
                className="px-4 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-700 disabled:bg-emerald-300 text-white text-xs font-bold shadow-xs transition-all cursor-pointer flex items-center gap-1"
              >
                <Save size={14} />
      ...
      {viewSavedRekap && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-4xl w-full p-4 sm:p-5 shadow-2xl border border-slate-100 space-y-3 max-h-[90vh] flex flex-col animate-in fade-in zoom-in-95 duration-150">
            <div className="flex items-center justify-between pb-2.5 border-b border-slate-100">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-500 font-medium m-0 flex items-center gap-1">
                    <Calendar size={11} />
                    <span>Disimpan: {new Date(viewSavedRekap.tanggal_rekap || viewSavedRekap.created_at || '').toLocaleString('id-ID')}</span>
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-1.5">
                <button
                  ...
                  className="px-2.5 py-1 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-xs font-bold shadow-xs transition-all cursor-pointer flex items-center gap-1"
                >
                  <Download size={13} />
                  <span>Ekspor Excel</span>
                </button>
            ...
            {/* Quick Summary Cards */}
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-1.5 bg-slate-50 p-2.5 rounded-xl border border-slate-200 text-center text-xs">
              <div>
            ...
            {/* Table Detail */}
            <div className="overflow-x-auto border border-slate-200 rounded-lg flex-1 max-h-[350px]">
              <table className="w-full text-left text-xs border-collapse">
                <thead className="bg-slate-100 text-slate-700 font-extrabold uppercase sticky top-0 z-10 border-b border-slate-200">
                  <tr>
                  ...
                  {(!viewSavedRekap.data_detail || viewSavedRekap.data_detail.length === 0) ? (
                    ...
                    viewSavedRekap.data_detail.map((it: any, idx: number) => (
                      <tr key={idx} className="hover:bg-slate-50 transition-colors">
                        <td className="p-2 text-center font-mono text-slate-400">{it.rekapNo || (idx + 1)}</td>
                        <td className="p-2 font-mono font-bold text-amber-600">{it.nomorSJ}</td>
                        <td className="p-2 whitespace-nowrap">{it.tanggal}</td>
                        <td className="p-2 font-semibold text-slate-900">{it.tujuan}</td>
                        <td className="p-2 font-semibold text-slate-900">{it.namaBarang}</td>
                        <td className="p-2 text-right font-mono font-bold">{formatNum(it.ctn)}</td>
                        <td className="p-2 text-right font-mono font-bold">{formatNum(it.pcs)}</td>
                        <td className="p-2 text-right font-mono">{(Number(it.berat) || 0).toFixed(2)}</td>
                        <td className="p-2 text-right font-mono">{(Number(it.kubikasi) || 0).toFixed(3)}</td>
                      </tr>
                    ))
                  )}
                </tbody>
            ...
            <div className="flex justify-end pt-1.5 border-t border-slate-100">
              <button
                type="button"
                onClick={() => setViewSavedRekap(null)}
                className="px-4 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold transition-all cursor-pointer"
              >
                Tutup
              </button>
            </div>
